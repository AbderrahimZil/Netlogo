...
houses-own [ reward
             lock   ; of power units to lay in order to break
             value ]

to lattice2D
  ca
  reset-ticks
  nw:generate-lattice-2d nodes links world-width world-height false
  (
    foreach (sort nodes) (sort patches) [ [t p] -> ask t [
    move-to p
...
to connect
    ask nodes [
    create-links-with other nodes in-radius 1 ]
end

to furnish
  ask nodes [
    if (xcor mod 2 = 1) and ( ycor mod 2 = 1 ) [
      set wall? true
      ask my-links [die]
      set color brown - 2]
      ]
...
;    ask link-neighbors with [wall? = false] [set targ targ + 1]
  ...
  ask n-of n_houses nodes with [ wall? = false and occupied? = false ][
    set occupied? true
    hatch 1 [ ;; sprout is for patch
      set size 1
      set color yellow
